...
Terra applies defense in depth across the API, gateway, and infrastructure
(SRS §29, §13.19, §13.20).
...
- `POST /api/v1/fetch`, `POST /api/v1/ask` — require a bearer token **when auth
  is enabled** (`TERRA_AUTH_ENABLED=true`).
...
`TERRA_AUTH_ENABLED=true` and requires `TERRA_AUTH_ADMIN_TOKEN` (compose fails
...
guarded by a bootstrap admin credential (`TERRA_AUTH_ADMIN_TOKEN`).
...
2. **Gateway limiter** (`app/api/deps.py`) — configurable per-IP fixed-window
   ...
   `TERRA_RATE_LIMIT_ENABLED` / `TERRA_RATE_LIMIT_PER_MINUTE`.
...
only when `TERRA_TRUST_PROXY_HEADERS=true` (set in the prod topology, where Nginx
...
- Request bodies are validated by Pydantic; coordinates are range-checked
  ...
  capped.
- A body-size guard rejects oversized payloads (`TERRA_MAX_REQUEST_BODY_BYTES`,
...
### Prompt-injection resistance (12-A)

The `/ask` question is the only model-visible free text and is treated as
untrusted input at both AI stages. It is fenced between delimiters that are
stripped from the question itself (no fence breakout), and both system prompts
pin the trust model: the retrieved-data block is the sole authority on which
values exist; question text cannot assert values, unmark unavailable fields, or
amend the rules. Deterministic backstops hold regardless of model behavior —
the planner is catalog-constrained (invented fields are dropped and metered),
citations only validate against the Dataset Registry, the unavailable-field set
is computed from fetch nulls (never from model output), and an answer claiming
a value for an unavailable field gets an explicit corrective note appended.
Worst case under a successful injection is misleading *prose*; it cannot mint a
fabricated cited datum. Regression tests: `app/tests/test_phase12_trust.py`.
...
entry. `TERRA_CORS_ORIGINS` (a JSON list) is only for a *separate* browser app on
...
`TERRA_CORS_ALLOW_CREDENTIALS` is **off** by default. Two invariants are enforced
in `validate_runtime()`:
...
`./secrets/terra-gee.json`), never baked into an image. The key currently
