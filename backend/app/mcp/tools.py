...
Pure async functions over a :class:`PrismClient` — deliberately free of any
...
The §34.1 tools are ``prism_earth_fetch`` and ``prism_earth_ask``; the ``meta_*``
...
async def ask_tool(client: PrismClient, *, lat: float, lng: float, question: str) -> dict[str, Any]:
...
async def meta_layers_tool(client: PrismClient) -> dict[str, Any]:
    """The domain layers (SRS §13.7)."""
    return await client.get("/meta/layers")


async def meta_presets_tool(client: PrismClient) -> dict[str, Any]:
